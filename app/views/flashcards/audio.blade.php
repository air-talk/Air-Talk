        ...
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content text-center">
                <div class="modal-body">
                    ...
                    <p>Use your spacebar or click to reveal the definition. Then left and right arrow keys or press buttons to submit.</p>
                    <div class="row">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($unansweredFlashcards as $flashcard)
                                <tr>
                                    <td class="card-name" data-card-id="{{{ $flashcard->id }}}" data-title="{{{ $flashcard->title }}}" data-definition="{{{ $flashcard->front }}}" data-front="{{{ $flashcard->back }}}" data-percent="0"><strong>{{$flashcard->title}}</strong></td>
                                    {{-- change later --}}
                                    <td>0</td>
                                    <td>0</td>
                                    <td>
                            ...
                            @foreach($answeredFlashcards as $flashcard)
                                <tr>
                                    <td class="card-name" data-card-id="{{{ $flashcard->id }}}" data-title="{{{ $flashcard->title }}}" data-definition="{{{ $flashcard->front }}}" data-front="{{{ $flashcard->back }}}" data-percent="{{{ floor($flashcard->pivot->correct / $flashcard->pivot->attempts * 100) }}}"><strong>{{{ $flashcard->title }}}</strong></td>
        ...
        $(".card-name").hover(
          function() {
            if($(this).data('percent') == '0'){
                $( '#sideWell' ).html( "<h2 class='centered'>" + $(this).data('title') + "</h2>" + "<hr><br><p>" + $(this).data('front') + "</p><br><hr><br>" +  "<div class='progress'><div class='progress-bar progress-bar-info' role='progressbar' aria-valuenow='0' aria-valuemin='0' aria-valuemax='0' style='width: 0%'><span class='sr-only'> 0% </span></div></div><br>" );
            }else{
                $( '#sideWell' ).html( "<h2 class='centered'>" + $(this).data('title') + "</h2>" + "<hr><br><p>" + $(this).data('front') + "</p><br><hr><br>" +  "<div class='progress'><div class='progress-bar progress-bar-info' role='progressbar' aria-valuenow='" + $(this).data('percent') + "' aria-valuemin='0' aria-valuemax='100' style='width:" + $(this).data('percent') + "%'><span class='sr-only'> " + $(this).data('percent') + "%</span> </div> </div><br>" );
            }

          }, function() {
            $( '#sideWell' ).html( '<h3 class="centered">Do you know the correct response?</h3><hr><h4 class="centered">Use your spacebar or click to reveal the answer. Then press the left or right arrow key or click the buttons to submit.</h4><hr><a type="button" class="btn btn-primary btn-circle col-md-offset-3" data-toggle="modal" data-target="#myModal">Practice Flashcards</a>'  );
          }
        );
